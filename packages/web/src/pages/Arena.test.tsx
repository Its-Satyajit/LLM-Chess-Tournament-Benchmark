...
describe('Arena page (modular)', () => {
  ...
  it('connects to match and loads game state', async () => {
    ...
    vi.mocked(api.getMatch).mockResolvedValue(mockMatch)
  ...
  it('transitions to next game when game_started WS event arrives', async () => {
    ...
    vi.mocked(api.getMatch).mockResolvedValueOnce(mockMatchG1)
    vi.mocked(api.getGameState).mockResolvedValueOnce(mockGameStateG1)
