...
describe('ChessGame', () => {
  ...
  describe('game over detection', () => {
    it('should detect checkmate', () => {
      ...
      expect(game.getResult()).toEqual({ winner: 'white', reason: 'checkmate' })
    })
    ...
    it('should detect stalemate', () => {
      ...
      const game = new ChessGame(fen)
      expect(game.isGameOver()).toBe(true)
      expect(game.getResult()).toEqual({ winner: null, reason: 'stalemate' })
    })
    ...
    it('should detect draw by insufficient material', () => {
      ...
      const game = new ChessGame(fen)
      expect(game.isGameOver()).toBe(true)
      expect(game.getResult()).toEqual({ winner: null, reason: 'insufficient_material' })
    })
  })

  describe('Chess960', () => {
    it('should generate Chess960 position from seed', () => {
      const game = ChessGame.fromChess960Seed(12345)
      const state = game.getGameState()
    ...
    it('should generate different positions for different seeds', () => {
      const game1 = ChessGame.fromChess960Seed(12345)
      const game2 = ChessGame.fromChess960Seed(67890)
      expect(game1.getGameState().fen).not.toBe(game2.getGameState().fen)
    })

    it('should generate same position for same seed', () => {
      const game1 = ChessGame.fromChess960Seed(12345)
      const game2 = ChessGame.fromChess960Seed(12345)
