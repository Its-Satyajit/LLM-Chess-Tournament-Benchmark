...
tester.run("anti-slop/no-known-value-widening", noKnownValueWideningRule, {
  ...
  invalid: [
    ...
    {
      code:
        `${prelude} type Open = Record<string, Command>; const source = { start: startCommand }; const commands: Open = source;`,
      errors: [error,],
    },
    {
      code:
        `${prelude} type Open = { [key: string]: Command }; const source = { start: startCommand }; const commands: Open = source;`,
      errors: [error,],
    },
    {
      code:
        `${prelude} type Open = { [key in string]: Command }; const source = { start: startCommand }; const commands: Open = source;`,
      errors: [error,],
    },
    {
      code:
        `${prelude} type Open = Readonly<Record<string, Command>>; const source = { start: startCommand }; const commands: Open = source;`,
      errors: [error,],
    },
    {
      code:
